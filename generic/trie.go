...
type Trie interface {
	...
	Search(string) []generic.T
	Remove(string) error
	Update(string, generic.T) error
}
...
func (t *trie) Add(key string, val generic.T) error {
	if key == "" {
		return emptyKey()
	}
...
func (t *trie) Get(key string) (result generic.T, ok bool) {
	if key == "" {
		return result, false
	}
...
func (t *trie) Search(key string) []generic.T {
	var inlineStack [16]stackNode
	var results []generic.T
...
func (t *trie) Remove(key string) error {
	...
	var zeroVal generic.T

	stack[tip].value = zeroVal
...
func (t *trie) Update(key string, val generic.T) error {
	if key == "" {
		return emptyKey()
	}
